...
import { loginController, registerController, meController } from './controller/UserController.js';
import {createProjectController, getProjectsController, deleteProjectsController, updateProjectController, getProjectController} from './controller/ProjectController.js'
import { createTaskController, getTaskController, getSubTasksController, deleteTaskController, updateTaskController, createSubTaskController } from './controller/TaskController.js';

const PATH = process.env.DATABASE_URL.replace('<password>', process.env.PASSWORD)

mongoose.connect(PATH).then(() => console.log('Db ok')).catch((err) => console.log('Db error', err))
...
app.get('/projects/:projectId/tasks/:parentTaskId/subtasks', checkAuth, getSubTasksController);
app.post('/projects/:projectId/tasks/:parentTaskId/subtasks', checkAuth, createSubTaskController);
