...
export const getProjectsController = async (req, res) => {
    try {
        const projects = await ProjectSchema.find({ user: req.userId }).select('-tasks -user');
...
export const deleteProjectsController = async (req, res) => {
    try {
        const project = await ProjectSchema.findOneAndDelete({ _id: req.params.id });
...
export const updateProjectController = async (req, res) => {
    try {
        ...
        const updatedProject = await ProjectSchema.findByIdAndUpdate(projectId, updateData, { new: true });
